...
export class VisitorManager {
    ...
    private options: VisitorStatsOptions;
     ...
     */
    private async waitForFirebase(maxWaitTime: number = 15000): Promise<boolean> {
     ...
     * @returns 방문자 통계 결과
     */
    async updateVisitorCount(): Promise<VisitorStatsResult> {
        try {
            ...
            if (this.options.enableSessionTracking) {
                ...
                const oldVisitorKeys = allKeys.filter(key => 
                    key.startsWith(this.options.sessionKeyPrefix || 'visitor_counted_') && 
                    key !== sessionKey
                );
            ...
            // 첫 방문자라면 시작 날짜 설정
            if (!startDate) {
                startDate = new Date().toISOString().split('T')[0] as string; // YYYY-MM-DD 형식
     ...
     * @returns 방문자 통계 결과
     */
    async loadVisitorCount(): Promise<VisitorStatsResult> {
        try {
            // Firebase가 준비될 때까지 기다림
            const firebaseReady = await this.waitForFirebase();
            if (!firebaseReady || !this.firebase || !this.firebase.db) {
    ...
    displayVisitorCount(count: number, startDate: string | null): void {
        ...
        for (const selector of selectors) {
            ...
            if (!countElement) {
                // document에서 직접 찾기
                countElement = document.querySelector(selector) as HTMLElement | null;
        ...
        if (!foundAny) {
            ...
            const observer = new MutationObserver((mutations, obs) => {
                if (observerFound) return; // 이미 찾았으면 중단
     ...
     * @returns 방문자 통계 결과
     */
    async updateProgressVisitorCount(): Promise<VisitorStatsResult> {
        try {
            // Firebase가 준비될 때까지 기다림
            const firebaseReady = await this.waitForFirebase();
            if (!firebaseReady || !this.firebase || !this.firebase.db) {
     ...
     * @returns 초기화 결과
     */
    async resetVisitorStats(): Promise<{ success: boolean; error?: string }> {
        try {
            if (!this.firebase || !this.firebase.db) {
                return {
                    success: false,
     ...
     * @returns 방문자 통계 데이터
     */
    async getCurrentStats(): Promise<VisitorStats | null> {
        try {
            if (!this.firebase || !this.firebase.db) {
                return null;
            }
 ...
 */
export function initializeVisitorManager(options?: VisitorStatsOptions): VisitorManager {
