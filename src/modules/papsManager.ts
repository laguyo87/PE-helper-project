...
export class PapsManager {
    ...
    private selectedStudentForChart: string | null = null; // 그래프에서 선택된 학생
    private currentRankingRecords: {record: number, name: string}[] | null = null; // 현재 랭킹 데이터 저장

    constructor(
        papsData: PapsData,
        $: (id: string) => HTMLElement,
        saveDataToFirestore: () => void,
        cleanupSidebar?: () => void  // 사용하지 않지만 호환성을 위해 유지
    ) {
    ...
    public onPapsInput(e: Event, cls: PapsClass): void {
        ...
        // 비동기로 처리하여 UI 블로킹 방지
        this.updateStudentShareData(st, cls, tr as HTMLTableRowElement).catch(error => {
    ...
    public calcPapsGrade(categoryId: string, value: number, gender: string, gradeLevel: string, cls: PapsClass): string {
        ...
        if (categoryId === 'bodyfat') {
            ...
            if (selectedTest === '팔굽혀펴기' && gender === '여자') selectedTest = '무릎대고팔굽혀펴기';
        }
        ...
        for (let i = ranges.length - 1; i >= 0; i--) {
            ...
            if (value >= min && value <= max) return typeof g === 'number' ? `${g}등급` : String(g);
        }
        return '';
    }
...
`;
    ...
    private drawStandardDeviationChart(sortedRecords: {record: number, name: string}[], studentName: string): void {
        ...
        for (let sigma = -3; sigma <= 3; sigma++) {
            if (sigma === 0) continue; // 평균선은 나중에 따로 그리기
     ...
     * @param tr 테이블 행 요소
     */
    private async updateStudentShareData(
        student: PapsStudent,
        cls: PapsClass,
    ...
    private showQRPrintModal(
        ...
        qrImages.forEach((img) => {
            img.addEventListener('click', () => {
                ...
                const closeBtnEl = overlay.querySelector('#qr-zoom-close-btn') as HTMLButtonElement | null;
                if (closeBtnEl) {
    ...
    private printQRCodes(
        ...
        let numberFontSize: string;
        let itemPadding: string;
        let gapSize: string;
     ...
     * @returns 저장된 반 목록
     */
    private getSavedQRClasses(): Array<{ classId: string; className: string; savedAt: string }> {
     ...
     */
    private showSavedQRListModal(): void {
        ...
        classButtons.forEach(btn => {
            ...
            btn.addEventListener('mouseenter', () => {
                (btn as HTMLElement).style.background = '#e9ecef';
                (btn as HTMLElement).style.borderColor = '#007bff';
            });
