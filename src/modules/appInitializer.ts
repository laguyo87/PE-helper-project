...
export class AppInitializer {
  ...
  private async initializePapsManager(papsData: PapsData): Promise<void> {
    ...
    try {
      ...
      logger.debug('window.papsManager 등록됨:', (window as any).papsManager);
      logger.debug('window.papsManager.selectPapsClass:', typeof (window as any).papsManager?.selectPapsClass);
    } catch (error) {
