...
export class ShareManager {
  ...
  private async fetchLatestShareData(shareId: string): Promise<SharedPapsStudentData | null> {
    try {
      const { doc, getDoc, collection, getDocs, db } = (window as any).firebase || {};
      ...
      let latestStudentData: any = null;
      ...
      if (currentUser) {
        ...
        const currentUserDoc = usersSnapshot.docs.find((doc: any) => doc.id === currentUser.uid);
        if (currentUserDoc) {
          const userData = currentUserDoc.data();
          if (userData.paps && userData.paps.classes && Array.isArray(userData.paps.classes)) {
            const matchingClass = userData.paps.classes.find((classData: any) => {
            ...
            if (matchingClass && matchingClass.students && Array.isArray(matchingClass.students)) {
              latestStudentData = matchingClass.students.find((student: any) => {
      ...
      if (!latestStudentData) {
        usersSnapshot.forEach((userDoc: any) => {
          ...
          if (userData.paps && userData.paps.classes && Array.isArray(userData.paps.classes)) {
            ...
            if (matchingClass && matchingClass.students && Array.isArray(matchingClass.students)) {
              const student = matchingClass.students.find((student: any) => {
                return student && (Number(student.id) === studentId || Number(student.studentId) === studentId);
              });
      ...
      if (latestStudentData) {
        ...
        // 최신 기록으로 공유 데이터 업데이트
        const updatedShareData: SharedPapsStudentData = {
          ...shareData,
  ...
  private async showPapsStudentRecord(shareData: SharedPapsStudentData, shareId?: string): Promise<void> {
    ...
    // 홈 화면에 추가 안내 함수
    function showHomeScreenAddGuide(url: string, studentName: string) {
      ...
      categories.forEach(categoryId => {
        ...
        if (categoryId === 'strength') {
          ...
          if (hasGripStrength) {
            ...
            const recordsWithNames: {record: number, name: string, studentId?: number}[] = [];
  ...
  private generateExercisePrescription(shareData: SharedPapsStudentData): string {
    ...
    Object.keys(shareData.grades).forEach(categoryId => {
      ...
      if (!grade || (record === undefined && categoryId !== 'bodyfat')) return;
   ...
   * @param shareData 공유 데이터
   */
  private showDebugLogs(gradeRankings: Record<string, string>, shareData: SharedPapsStudentData): void {
    // 기존 디버그 로그 제거
    ...
    const debugInfo = (window as any).__rankingDebugInfo || {};
