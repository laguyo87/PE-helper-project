...
export class DataSyncService {
  ...
  private stateManager: AppStateManager | null;
  private storageKey: string;
  private getDefaultData: () => Partial<AppState>;
   ...
   * Firestore에서 데이터 로드
   */
  public async loadFromFirestore(): Promise<SyncResult> {
    ...
    try {
      ...
      if (!firebaseReady || !(window as any).firebase || !(window as any).firebase.db) {
   ...
   * 로컬 스토리지에서 데이터 로드
   */
  public loadFromLocal(): SyncResult {
   ...
   * Firestore에 데이터 저장
   */
  public async saveToFirestore(): Promise<SyncResult> {
   ...
   * 로컬 스토리지에 데이터 저장
   */
  public saveToLocal(data?: Partial<AppState>): SyncResult {
   ...
   * 로드된 데이터 처리 및 검증
   */
  private processLoadedData(appData: AppData): Partial<AppState> {
    // 기본 구조로 데이터 처리
    const processedData: Partial<AppState> = {
   ...
   * DataManager 설정
   */
  public setDataManager(dataManager: DataManager | null): void {
   ...
   * AuthManager 설정
   */
  public setAuthManager(authManager: AuthManager | null): void {
   ...
   * AppStateManager 설정
   */
  public setStateManager(stateManager: AppStateManager | null): void {
 ...
 */
export function createDataSyncService(options: DataSyncServiceOptions): DataSyncService {
