...
export class TournamentManager {
    ...
    private dataUpdateCallback: ((newData: TournamentData) => void) | null = null;
     ...
     * @param data 토너먼트 데이터
     */
    public setTournamentData(data: TournamentData): void {
     ...
     */
    public setDataUpdateCallback(callback: (newData: TournamentData) => void): void {
     ...
     * 토너먼트 UI를 렌더링합니다.
     */
    public renderTournamentUI(): void {
     ...
     */
    public renderTournamentDashboard(tourney: Tournament): void {
        ...
        this.$('#teamNameInput').addEventListener('keypress', (e: KeyboardEvent) => {
            if (e.key === 'Enter') {
                e.preventDefault();
    ...
    public createTournament(): void {
        ...
        const input = this.getElement('#tournamentNameInput') as HTMLInputElement;
        if (!input) return;
        ...
        // 검증 통과 후 추가
        const newTourney: Tournament = validation.data!;
     ...
     * @param id 토너먼트 ID
     */
    public selectTournament(id: string): void {
     ...
     */
    public showTournamentSettings(tournamentId: string): void {
     ...
     * 토너먼트 설정을 업데이트합니다.
     */
    public updateTournamentSettings(): void {
        ...
        const nameInput = this.getElement('#tourneyName') as HTMLInputElement;
        const sportInput = this.getElement('#tourneySport') as HTMLInputElement;
        const formatInputs = document.querySelectorAll('input[name="format"]') as NodeListOf<HTMLInputElement>;
        const seedingInputs = document.querySelectorAll('input[name="seeding"]') as NodeListOf<HTMLInputElement>;
        
        if (nameInput) tourney.name = nameInput.value.trim();
        if (sportInput) tourney.sport = sportInput.value.trim();
        
        formatInputs.forEach(input => {
            if (input.checked) tourney.format = input.value as 'single' | 'double';
        });
        
        seedingInputs.forEach(input => {
            if (input.checked) tourney.seeding = input.value as 'input' | 'random';
        });
    ...
    public addTeamToTournament(): void {
        ...
        const input = this.getElement('#teamNameInput') as HTMLInputElement;
        if (!input) return;
     ...
     */
    public removeTeamFromTournament(teamNameToRemove: string): void {
     ...
     * @param tourney 토너먼트 객체
     */
    public buildBracket(tourney?: Tournament): void {
        ...
        let roundsData: TournamentMatch[][] = [];

        if (teams.length > 0) {
            let matchIdSeq = 1;
            const makeMatch = (roundIdx: number, slotIdx: number, teamA: string | null, teamB: string | null, isBye: boolean = false): TournamentMatch => ({
     ...
     * @param value 점수
     */
    public onScoreInputTournament(matchId: string, side: 'A' | 'B', value: string): void {
     ...
     * @param tourney 토너먼트 객체
     */
    public propagateWinners(tourney: Tournament): void {
        if (!tourney || !Array.isArray(tourney.rounds)) return;
     ...
     */
    public renderBracket(tourney: Tournament, isReadOnly: boolean = false): void {
        ...
        if (!roundsEl || !svgEl || !Array.isArray(roundsData) || roundsData.length === 0) {
            if(roundsEl) roundsEl.html('<div style="text-align:center; padding: 2rem; color: var(--ink-muted);">팀을 추가하여 대진표 생성을 시작하세요.</div>');
            if(svgEl) svgEl.html('');
            return;
        }
     ...
     * @returns HTML 문자열
     */
    public renderMatchCard(match: TournamentMatch, rIdx: number, tourney: Tournament, isReadOnly: boolean = false): string {
        // 팀 표시 로직 개선
        ...
        if (finalRoundIdx >= 0 && roundsData[finalRoundIdx][0].winner) {
            ...
            if (rIdx === finalRoundIdx) {
                if (match.teamA === winner) rankBadgeA = this.getMedal('gold');
                if (match.teamB === winner) rankBadgeB = this.getMedal('gold');
                if (match.teamA === runnerUp) rankBadgeA = this.getMedal('silver');
                if (match.teamB === runnerUp) rankBadgeB = this.getMedal('silver');
            }
            ...
            if (roundsData.length > 1 && rIdx === finalRoundIdx - 1) {
                ...
                if (semiFinalLosers.includes(match.teamA)) rankBadgeA = this.getMedal('bronze');
                if (semiFinalLosers.includes(match.teamB)) rankBadgeB = this.getMedal('bronze');
            }
        }
        ...
        // 원래 팀 렌더링 함수
        const renderTeam = (team: string | null, teamType: 'A' | 'B', rankBadge: string, isWinner: boolean, isLoser: boolean, isBye: boolean) => {
     ...
     * @returns 라운드 라벨 배열
     */
    private makeRoundLabels(count: number): string[] {
        if(count <= 0) return []; 
     ...
     * @returns 메달 HTML
     */
    private getMedal(type: 'gold' | 'silver' | 'bronze'): string {
     ...
     * @param tourney 토너먼트 객체
     */
    private drawSvgLines(tourney: Tournament): void {
        const svg = this.getElement('#svgLayer') as unknown as SVGElement;
     ...
     * @returns 좌표 객체
     */
    private getBottomRight(el: Element, container: Element): { x: number, y: number } {
     ...
     * @returns 좌표 객체
     */
    private getBottomLeft(el: Element, container: Element): { x: number, y: number } {
        const r1 = el.getBoundingClientRect(); 
        const rC = container.getBoundingClientRect(); 
 ...
 */
export function initializeTournamentManager(tournamentData: TournamentData, saveCallback?: (() => void) | null): TournamentManager {
