 ...
 * 사용자에게 에러 메시지를 표시합니다.
 */
export function showError(error: unknown, elementId?: string): void {
 ...
 * 사용자에게 성공 메시지를 표시합니다.
 */
export function showSuccess(message: string, elementId?: string): void {
  if (elementId) {
    const element = document.getElementById(elementId);
    if (element) {
      element.textContent = message;
 ...
 * 사용자에게 경고 메시지를 표시합니다.
 */
export function showWarning(message: string, elementId?: string): void {
  if (elementId) {
    const element = document.getElementById(elementId);
    if (element) {
      element.textContent = message;
 ...
 */
export function safeExecuteWithDefault<T>(
  fn: () => T,
  defaultValue: T
): T {
  try {
 ...
 * @returns 함수 실행 결과 또는 기본값
 */
export async function safeAsyncExecuteWithDefault<T>(
  fn: () => Promise<T>,
  defaultValue: T
): Promise<T> {
  try {
