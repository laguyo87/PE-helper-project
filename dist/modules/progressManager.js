...
export class ProgressManager {
    ...
    setupProgressSheetEventListeners() {
        ...
        progressSheetArea.addEventListener('input', (e) => {
            ...
            if (target.dataset.week && target.dataset.session) {
                this.updateProgressSession(parseInt(target.dataset.week), parseInt(target.dataset.session), target.value, undefined);
            }
        });
    }
    /**
     ...
     * 진도표 세션 업데이트
     */
    updateProgressSession(week, session, value, completed) {
        const selectedClass = this.getProgressSelected();
        if (!selectedClass)
            return;
        ...
        const inputElement = this.$(`[data-week="${week}"][data-session="${session}"]`);
        if (inputElement) {
