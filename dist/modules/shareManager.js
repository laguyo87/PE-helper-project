...
export class ShareManager {
     ...
     */
    constructor(options = {}) {
        this.realtimeUnsubscribers = new Map(); // shareId별 실시간 리스너 정리 함수
        this.lastUpdatedData = new Map(); // shareId별 마지막 업데이트된 데이터
        this.updateTimeouts = new Map(); // shareId별 디바운스 타이머
    ...
    `;
      ...
      `;
        ...
        if (shareId) {
            try {
                const { doc, onSnapshot, db } = window.firebase || {};
                if (db && doc && onSnapshot) {
                    ...
                    // 실시간 리스너 등록
                    const unsubscribe = onSnapshot(shareDocRef, async (snapshot) => {
                        if (!snapshot.exists()) {
                    ...
                    if (targetUserData.paps && targetUserData.paps.classes && Array.isArray(targetUserData.paps.classes)) {
                        ...
                        targetUserData.paps.classes.forEach((classData) => {
                            if (classData && classData.gradeLevel === shareData.gradeLevel && classData.students) {
                                const classStudentsWithGender = classData.students.filter((s) => s && s.gender === shareData.studentGender);
                                totalWithGender += classStudentsWithGender.length;
                                const classStudentsWithRecord = classData.students.filter((student) => {
                                    totalChecked++;
                                    if (student && student.gender === shareData.studentGender) {
                                        const record = student.records?.[categoryId];
                                        if (record !== undefined && record !== null) {
