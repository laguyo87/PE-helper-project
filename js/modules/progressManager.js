...
export class ProgressManager {
     ...
     * @param config 설정 옵션
     */
    constructor(progressClasses, selectedClassId, $, $$, saveDataCallback, config = {
        enableAutoSave: true,
     ...
     */
    addProgressClass() {
        const input = this.$('#progressClassNameInput');
        if (!input)
            return;
    ...
    saveProgressSetting() {
        ...
        if (!selectedClass)
            return;
        const teacherName = this.$('#progressTeacherName')?.value || '';
        const unitContent = this.$('#progressUnitContent')?.value || '';
        const weeklyHours = parseInt(this.$('#progressWeeklyHours')?.value || '2');
     ...
     */
    generateProgressSheet(selectedClass) {
    ...
    setupEventListeners() {
        ...
        // 반 이름 입력 엔터키
        const nameInput = this.$('#progressClassNameInput');
        if (nameInput) {
        ...
        if (classList) {
            classList.addEventListener('click', (e) => {
                ...
                if (classItem && !actionBtn) {
                    ...
                    if (classId)
                        this.selectProgressClass(classId);
                }
                else if (actionBtn) {
    ...
    loadProgressSelected() {
        ...
        if (!selectedClass)
            return;
        // 설정 폼에 데이터 채우기
        const teacherNameInput = this.$('#progressTeacherName');
        const unitContentInput = this.$('#progressUnitContent');
        const weeklyHoursSelect = this.$('#progressWeeklyHours');
        if (teacherNameInput)
 ...
 */
export function initializeProgressManager(progressClasses, selectedClassId, $, $$, saveDataCallback, config) {
